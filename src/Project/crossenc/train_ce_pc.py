"""Train cross-encoder for post–criterion (chunked) classification."""
        ...
        for chunk in chunks:
            examples.append(
                PairExample(
                    ...
                    criterion=crit.desc,
                    label=lab.label,
                    meta={"post_id": lab.post_id, "cid": lab.cid, "chunk_id": chunk["chunk_id"]},
                )
            )
    return examples


def train_ce_pc(cfg: DictConfig) -> Path:
    ...
    train_cfg = TrainingConfig(
        ...
        max_length=cfg.data.chunk_max_len,
    ...
    ckpt_path = ckpt_dir / "ce_pc.pt"
    torch.save(model.state_dict(), ckpt_path)
    save_config(cfg, out_dir)
    logger.info("Saved CE-PC checkpoint to %s", ckpt_path)
    return ckpt_path
